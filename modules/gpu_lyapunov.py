...
def _maping(x, y, l, a):
    ...
    newx = (x**2 * (l * ((x**2 + y**2)**(a/2) - 1) + 2) - l *
            y**2 * ((x**2 + y**2)**(a/2) - 1))/(x**2 + y**2)
...
def _j1(X, Y, L, A, J):
    ...
    if (pos < len_t):
        for i in range(n):
            ...
            l = L[i]
            a = A[i]
            J[pos, i] = x*a*l*(x**2+y**2)**(a/2-2)*(x**2-y**2) + (1-l+l*(x**2+y**2)
                                                                  ** (a/2))*(2*x/(x**2+y**2) - 2*x*(x**2-y**2)/(x**2+y**2)**2)
...
def _j2(X, Y, L, A, J):
    ...
    if (pos < len_t):
        for i in range(n):
            ...
            l = L[i]
            a = A[i]
            J[pos, i] = y*a*l*(x**2+y**2)**(a/2-2)*(x**2-y**2) + (1-l+l*(x**2+y**2)
                                                                  ** (a/2))*(-2*y/(x**2+y**2) - 2*y*(x**2-y**2)/(x**2+y**2)**2)
...
def _j3(X, Y, L, A, J):
    ...
    if (pos < len_t):
        for i in range(n):
            ...
            l = L[i]
            a = A[i]
            J[pos, i] = 2*y*x**2*a*l*(x**2+y**2)**(a/2-2) + (
                1-l+l*(x**2+y**2)**(a/2))*(2*y/(x**2+y**2)-4*x**2*y/(x**2+y**2)**2)
...
def _j4(X, Y, L, A, J):
    ...
    if (pos < len_t):
        for i in range(n):
            ...
            l = L[i]
            a = A[i]
            J[pos, i] = 2*x*y**2*a*l*(x**2+y**2)**(a/2-2) + (
                1-l+l*(x**2+y**2)**(a/2))*(2*x/(x**2+y**2)-4*y**2*x/(x**2+y**2)**2)
...
class system:
    ...
    def __init__(self, x, y, l, a, n_transient, n_attractor):
        ...
        print('Iterating over the transient of {:.0f} steps'.format(
            n_transient))
        t = time.time()
        X, Y = self.repeatmap(n=self.n_transient, nosave=True)
        print('    Done in {:.2f} s'.format(time.time() - t))

        print('Iterating over the attractor of {:.0f} steps'.format(
            n_attractor))
        t = time.time()
        self.X_attractor, self.Y_attractor = self.repeatmap(
            x=X, y=Y, n=int(n_attractor), nosave=False)
        print('    Done in {:.2f} s'.format(time.time() - t))

        print('Generating initial conditions')
        t = time.time()
        self.phi = np.random.uniform(
            0, 2*np.pi, list(self.X_attractor.shape)[1:])
        self.E = np.array([[np.cos(self.phi), -np.sin(self.phi)],
        ...
        self.J = self.jacobian(
            self.X_attractor, self.Y_attractor, self.l, self.a)
        print('    Done in {:.2f} s'.format(time.time() - t))
    ...
    def calcLyapunov(self):
        ...
        for t in tqdm(range(n_attractor)):

            # Subsetting the jacobian matrix
            jacobian_matrix = J[t]

            # rotating the angle matrix
            angle_matrix = np.einsum(
                'xylaij,jkxyla->ikxyla', jacobian_matrix, angle_matrix)

            # Calculating the Lyapunov Exponents
            first_exponent[t] = np.linalg.norm(angle_matrix[:, 0], axis=0)
            sum_of_exponents[t] = np.linalg.det(
                np.einsum('ijxyla->xylaij', angle_matrix))
    ...
    def plot_Lyapunov_1(self, savefig=True, figname=None):
        ...
        fig, ax = plt.subplots()
        divnorm = colors.DivergingNorm(vmin=lyapunov_1.min(axis=0).min(
            axis=0).min(), vcenter=0, vmax=lyapunov_1.max())
        plt.contourf(a[0, 0, :, :], l[0, 0, :, :], lyapunov_1.min(
            axis=0).min(axis=0), levels=100, cmap='RdBu_r', norm=divnorm)
        cbar = plt.colorbar()
        for i in range(lyapunov_1.shape[0]):
            for j in range(lyapunov_1.shape[1]):
                plt.contour(a[0, 0, :, :], l[0, 0, :, :], lyapunov_1[i, j], levels=[
                            0, ], colors=('k',), alpha=0.1)
        contour = plt.contour(a[0, 0, :, :], l[0, 0, :, :], lyapunov_1.max(
            axis=0).max(axis=0), levels=[0, ], colors=('blue',), alpha=1)
    ...
    def plot_Lyapunov_2(self, savefig=True, figname=None):
        ...
        divnorm = colors.DivergingNorm(vmin=np.nanmin(np.nanmax(
            np.nanmax(lyapunov_2, axis=0), axis=0)), vcenter=0, vmax=np.nanmax(lyapunov_2))
        plt.contourf(a[0, 0, :, :], l[0, 0, :, :], np.nanmax(np.nanmax(
            lyapunov_2, axis=0), axis=0), levels=100, cmap='RdBu_r', norm=divnorm)
        cbar = plt.colorbar()
        ...
        for i in range(lyapunov_2.shape[0]):
            for j in range(lyapunov_2.shape[1]):
                plt.contour(a[0, 0, :, :], l[0, 0, :, :], lyapunov_2[i, j], levels=[
                            0, ], colors=('k',), alpha=0.1)
        lyap_sum = plt.contour(a[0, 0, :, :], l[0, 0, :, :], lyapunov_2.max(
            axis=0).max(axis=0), levels=[0, ], colors=('blue',), alpha=1)
    ...
    def new_coords(self, da=0.01, mode=2):
        ...
        if mode == 2:
            ...
            for i in range(self.lyapunov_2.shape[2]):
                j = np.argwhere(
                    np.diff(np.sign(self.lyapunov_2.max(axis=0).max(axis=0)[i, :])))

                if j != []:
                    indicies += [[i, j[0, 0]]]
                    a1[:, :, i, :] = np.linspace(
                        self.a[0, 0, 0, j[0, 0]]-5*da, self.a[0, 0, 0, j[0, 0]]+5*da, self.a.shape[-1])
        ...
        if mode == 1:
            ...
            for i in range(self.lyapunov_1.shape[2]):
                j = np.argwhere(
                    np.diff(np.sign(self.lyapunov_1.max(axis=0).max(axis=0)[i, :])))
                if j != []:
                    indicies += [[i, j[0, 0]]]
                    a1[:, :, i, :] = np.linspace(
                        self.a[0, 0, 0, j[0, 0]]-5*da, self.a[0, 0, 0, j[0, 0]]+5*da, self.a.shape[-1])
...
#         divnorm = colors.DivergingNorm(vmin=max([np.nanmin(np.nanmax(np.nanmax(system.lyapunov_2, axis=0), axis=0)) for system in systems]), vcenter=0, vmax=max[np.nanmax(system.lyapunov_2) for system in systems])
        ...
        for system in systems:
            ...
            plt.contourf(a[0, 0, :, :], l[0, 0, :, :], np.nanmax(
                np.nanmax(lyapunov_2, axis=0), axis=0), levels=100, cmap='RdBu_r')
#                          , norm=divnorm)
            for i in range(lyapunov_2.shape[0]):
                for j in range(lyapunov_2.shape[1]):
                    plt.contour(a[0, 0, :, :], l[0, 0, :, :], lyapunov_2[i, j], levels=[
